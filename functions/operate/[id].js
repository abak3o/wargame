...
export async function onRequest(context) {
  ...
  try {
    ...
    if (opRes && opRes.ok) {
      ...
      const arr = Array.isArray(operates) ? operates : Object.values(operates || {});
      article = arr.find(a => String(a.id) === String(id) || String(a.slug) === String(id)) || arr[0] || null;
    }
  ...
  if (!article) {
    article = {
      ...
      body: `これはサンプル記事です。ID=${id || 'sample'} の内容がここに表示されます。`,
      author: 'system',
      is_published: true
    };
  }
  ...
  const keys = [article && article.id, article && article.slug, article && article.external_id].filter(Boolean).map(String);
  const filtered = (questions || []).filter(q => {
    const qOperateId = q && q.operate_id != null ? String(q.operate_id) : null;
    const qExternal = q && q.operate_external_id != null ? String(q.operate_external_id) : null;
    return (qOperateId && keys.includes(qOperateId)) || (qExternal && keys.includes(qExternal));
  });
